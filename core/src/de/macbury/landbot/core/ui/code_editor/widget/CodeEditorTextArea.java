 ...
 */
public class CodeEditorTextArea extends WidgetGroup implements Disposable {
      ...
      public boolean mouseMoved(InputEvent event, float x, float y) {
        if (isFocused()) {
          if (x <= GUTTER_WIDTTH || (x >= scroll.getWidth() - scroll.getScrollBarWidth()) || (y <= scroll.getHeight() - scroll.getScrollHeight())) {
            messages.dispatchShowNormalCursor();
            //HashBot.ui.normalCursor();
            //Gdx.graphics.setSystemCursor(Cursor.SystemCursor.Arrow);
          } else {
            messages.dispatchShowTextCursor();
  ...
  private boolean keyTyped(InputEvent event, char character) {
    if (isFocused()) {
      if (event.getKeyCode() == Input.Keys.TAB) {
        caret.clearSelection();
        insertText(getTabInSpaces());
        caret.incCol(getTabWidth());
      } else if (event.getKeyCode() == Input.Keys.FORWARD_DEL) {
        delete();
      } else if (event.getKeyCode() == Input.Keys.BACKSPACE) {
        backspace();
      } else if (event.getKeyCode() == Input.Keys.ENTER) {
        insertText("\n");
        caret.incRow();
        ...
        insertText(out);
        caret.setCol(spaces);
      } else if (Character.isAlphabetic(character) || Character.isDigit(character) || Character.isWhitespace(character) || specialChars.contains(String.valueOf(character))) {
  ...
  private void updateAutocomplete() {
    ...
    if (currentLine != null) {
      Element element = currentLine.getElementAt(caret.getCol());
      if (element == null) {
        Gdx.app.log(TAG, "nothing..");
      } else {
        Gdx.app.log(TAG, element.text + " => " + element.kind);
      }
    }
  }
  ...
  private void pushHistory() {
    history.add(text);//TODO save caret position too
  }
  ...
  public void draw(Batch batch, float parentAlpha) {
    ...
    style.lineNumberBackground.draw(batch, x, y, GUTTER_WIDTTH, totalHeight);
    ...
    for(Line line : lines) {
      ...
      if (isFocused() && line.getLineNumber() == caret.getRow()+1) {
        style.focusedLineBackground.draw(batch, 0, linePosY - PADDING_VERITICAL, totalWidth, lineHeight);
      }

      glyphLayout.setText(style.font, line.getLineString());
      style.font.setColor(style.lineNumberColor);
      style.font.draw(batch, line.getLineString(), gx - glyphLayout.width - GUTTER_PADDING, linePosY);
      ...
      for (int lx = 0; lx < line.size; lx++) {
        ...
        glyphLayout.setText(style.font, elem.text);
  ...
  public static class CodeEditorTextAreaStyle extends TextField.TextFieldStyle {
    public Drawable lineNumberBackground;
    public Drawable exceptionGutterIcon;
    public Drawable focusedLineBackground;
